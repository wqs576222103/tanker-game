...
function setDirName(t, name) {
  t.dir = { ...DIRS[name] };
  t.dirName = name;
}

function tryTeleport(t) {
  const cx = t.x + t.w / 2,
    cy = t.y + t.h / 2;
...
// ====================== 子弹 ======================
function fireBullet(x, y, dir, owner, dmg) {
...
function fireDir(x, y, angle, owner, dmg) {
  fireBullet(x, y, { x: Math.cos(angle), y: Math.sin(angle) }, owner, dmg);
}
...
function dirFree(t, name) {
  const d = DIRS[name];
  return !blocked(t.x + d.x * 4, t.y + d.y * 4, t.w, t.h, t);
}

// 该方向可直行的距离（像素），用于挑选最宽敞的出口
function freeDist(t, name) {
  const d = DIRS[name];
  let dist = 0;
  while (dist < 120) {
    dist += 2;
    if (blocked(t.x + d.x * dist, t.y + d.y * dist, t.w, t.h, t)) break;
  }
  return dist;
}

// 选择敌人方向：优先朝向玩家，被堵时改选可通行且前方最宽敞的方向，避免卡在角落
function pickEnemyDir(t) {
  const p = player;
  const dx = p.x - t.x,
    dy = p.y - t.y;
  ...
  if (dirFree(t, name)) return name;
  let best = name,
    bestD = -1;
  for (const n of DIR_NAMES) {
    if (!dirFree(t, n)) continue;
    const fd = freeDist(t, n);
    if (fd > bestD) {
      bestD = fd;
      best = n;
    }
...
function updateEnemies(dt) {
  ...
  for (const t of alive) {
    t.dirTimer -= dt;
    if (t.dirTimer <= 0) {
      setDirName(t, pickEnemyDir(t));
      t.dirTimer = 0.5 + Math.random() * 1.1;
    }
    const d = t.dir;
    const px = t.x,
      py = t.y;
    moveTank(t, d.x, d.y, dt);
    if (t.x === px && t.y === py) {
    ...
    if (blocked(t.x + d.x * 2, t.y + d.y * 2, t.w, t.h, t)) {
      setDirName(t, pickEnemyDir(t));
      t.stuckT = 0;
...
function updatePlayer(dt) {
  ...
  if (mx !== 0 || my !== 0) {
    ...
    moveTank(player, mx / len, my / len, dt);
  }
  ...
  if (k.fire && player.fireCd <= 0) {
    ...
    if (player.spreadT > gtMs) {
      for (let i = -1; i <= 1; i++)
        fireDir(cx, cy, ang + i * 0.18, "player", 1);
    } else {
      fireDir(cx, cy, ang, "player", 1);
    }
  }
  ...
  for (let i = drones.length - 1; i >= 0; i--) {
    ...
    const orbitA = dr.age * 2 + (i * Math.PI * 2) / Math.max(1, drones.length);
