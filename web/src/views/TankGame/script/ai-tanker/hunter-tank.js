...
export default {
  ...
  isClearDir(ctx, px, py, d) {
    ...
    const c2 = Math.min(ctx.COLS - 1, Math.floor((nx + w - 1) / CELL));
    const r1 = Math.max(0, Math.floor(ny / CELL));
    const r2 = Math.min(ctx.ROWS - 1, Math.floor((ny + h - 1) / CELL));
    for (let r = r1; r <= r2; r++) {
      for (let c = c1; c <= c2; c++) {
        const v = ctx.mapAt(c, r);
    ...
    for (const e of ctx.enemies) {
      ...
      if (ec.c >= c1 && ec.c <= c2 && ec.r >= r1 && ec.r <= r2) return false;
    }
    if (ctx.boss) {
      const bc = this.cell(
        ctx.boss.x + ctx.boss.w / 2,
      ...
      if (bc.c >= c1 && bc.c <= c2 && bc.r >= r1 && bc.r <= r2) return false;
    }
    return true;
  },
  ...
  // 该方向可直行的格子数（宽度感知，用逐步步进 + 包围盒检查）
  freeDistCells(ctx, px, py, d) {
    const E = ctx.TILE.EMPTY,
      G = ctx.TILE.GRASS;
    let n = 0,
      dist = 0;
    const step = Math.max(4, Math.floor(ctx.CELL / 4));
    while (dist < 200) {
      dist += step;
      const nx = px + d.x * dist,
        ny = py + d.y * dist;
      const c1 = Math.max(0, Math.floor(nx / ctx.CELL));
      const c2 = Math.min(
        ctx.COLS - 1,
        Math.floor((nx + ctx.player.w - 1) / ctx.CELL),
      );
      const r1 = Math.max(0, Math.floor(ny / ctx.CELL));
      const r2 = Math.min(
        ctx.ROWS - 1,
        Math.floor((ny + ctx.player.h - 1) / ctx.CELL),
      );
      let blocked = false;
      for (let r = r1; r <= r2 && !blocked; r++)
        for (let c = c1; c <= c2 && !blocked; c++) {
          const v = ctx.mapAt(c, r);
          if (v !== E && v !== G) blocked = true;
        }
      if (blocked) break;
      // 每经过一整格计数
      if ((dist + step) % ctx.CELL < step) n++;
    }
    return n;
  },
  ...
  isBlocked(dir) {
    ...
    const CELL = this.ctx.CELL;
    const w = player.w,
      h = player.h;
    const nx = player.x + (dir.x || 0) * CELL;
    const ny = player.y + (dir.y || 0) * CELL;
    const c1 = Math.max(0, Math.floor(nx / CELL));
    const c2 = Math.min(this.ctx.COLS - 1, Math.floor((nx + w - 1) / CELL));
    const r1 = Math.max(0, Math.floor(ny / CELL));
    const r2 = Math.min(this.ctx.ROWS - 1, Math.floor((ny + h - 1) / CELL));
    for (let r = r1; r <= r2; r++) {
      for (let c = c1; c <= c2; c++) {
        const v = map[r] ? map[r][c] : this.ctx.TILE.BORDER;
        if (v !== this.ctx.TILE.EMPTY && v !== this.ctx.TILE.GRASS) return true;
      }
    }
    return false;
  },
  ...
  getFreeDistance(dir) {
    ...
    while (dist < 300) {
      ...
      for (let r = r1; r <= r2 && !blocked; r++)
        for (let c = c1; c <= c2 && !blocked; c++) {
          const v = map[r] ? map[r][c] : this.ctx.TILE.BORDER;
          if (v !== this.ctx.TILE.EMPTY && v !== this.ctx.TILE.GRASS)
            blocked = true;
        }
      if (blocked) break;
    }
