...
在 `decide(ctx, dt)` 中返回 `mine: true` 即可在当前坦克所在位置安放地雷。

- **前提**：`player.mines > 0`。地雷数量通过拾取 `mine` 道具获得（每次 +3）。
- **落点**：地雷安放在坦克中心所在格子（`cellOf(x + w/2, y + h/2)`）处，地雷左上角与格子左上角对齐。
- **限制**：仅当该格子为 `TILE.EMPTY` 时才能安放（墙/碎石墙/草丛/传送门/边界处均不可）。
- **节流**：每次安放后该帧即被消耗（内部自动复位），连续布雷需在后续帧再次返回 `mine: true`。
- **效果**：敌方坦克碾上地雷即爆炸，对 3×3 范围内的敌方坦克造成 6 点伤害，并波及 3×3 格的碎石墙；不会伤害玩家自己的坦克，对 Boss 无效。
- **注意**：在 AI 对决模式中，地雷同样对其它 AI 坦克生效。
